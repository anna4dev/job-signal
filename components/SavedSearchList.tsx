  ...
  return (
    <div className="space-y-3">
      {savedSearches.map((item: SavedSearchItem) => (
        <div
          key={item.id}
          className="group p-3 bg-white border border-slate-100 rounded-xl hover:border-blue-200 hover:shadow-sm transition-all cursor-pointer"
          onClick={() => handleApply(item.filters)}
        >
          <div className="flex justify-between items-start mb-1">
            <h4 className="text-xs font-bold text-slate-800 group-hover:text-blue-600 truncate flex-1">
              {item.name}
            </h4>
            <button
              onClick={(e) => {
